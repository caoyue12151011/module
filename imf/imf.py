...
class Kroupa_PMF(MassFunction):
    # Present-day mass function derived with Kroupa's mass function, 
    # PMF(m)~IMF(m)*t(m), where t(m)~m^-2.5
    # kroupa = BrokenPowerLaw(breaks={0.08: -0.3, 0.5: 1.3, 'last': 2.3}, mmin=0.03, mmax=120)
    default_mmin = 0.03
    default_mmax = 120
    ...
    def integrate(self, mlow, mhigh, numerical=False):
        ...
        if mhigh <= mlow:
            raise ValueError("Must have mlow < mhigh in integral")
        if numerical:
            return super(Kroupa_PMF, self).integrate(mlow, mhigh)
    ...
    def m_integrate(self, mlow, mhigh, numerical=False, **kwargs):
        ...
        if numerical:
            return super(Kroupa_PMF, self).m_integrate(mlow, mhigh, **kwargs)
        else:
            distr1 = distributions.BrokenPowerLaw(
                [-self.p1 + 1, -self.p2 + 1, -self.p3 + 1],
                [self.mmin, self.break1, self.break2, self.mmax])
...
salpeter = Salpeter()
kroupa = Kroupa()
kroupa_PMF = Kroupa_PMF()
chabrier = Chabrier()
chabrier2005 = Chabrier2005()

massfunctions = {'kroupa': Kroupa, 'kroupa_PMF': Kroupa_PMF,
    'salpeter': Salpeter, 'chabrier': Chabrier,'chabrier2005': Chabrier2005}
...
# dont use it, seems to be wrong -- Yue Cao
